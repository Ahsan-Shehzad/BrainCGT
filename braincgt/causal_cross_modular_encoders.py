...
class CausalCrossAttentionLayer(nn.Module):
    """
    Inter-module causal cross-attention over directed edges (i -> j) where module_id differs.
    ...
    def forward(self, H: torch.Tensor, edge_index: torch.Tensor, edge_attr: torch.Tensor, module_id: torch.Tensor) -> torch.Tensor:
        src = edge_index[0]
        dst = edge_index[1]
        inter_mask = (module_id[src] != module_id[dst])
        if inter_mask.sum() == 0:
            return H

        src = src[inter_mask]
        dst = dst[inter_mask]
        w = edge_attr[inter_mask].view(-1)

        q = self.q_proj(H).view(-1, self.num_heads, self.attn_dim)
        ...
        bias = torch.sigmoid(self.gamma * (w.unsqueeze(-1) * self.w_a.unsqueeze(0) + self.b_a.unsqueeze(0)))
        logits = dot + bias
        ...
        out = []
        for h in range(self.num_heads):
            msg = v_e[:, h, :] * alpha[:, h].unsqueeze(-1)
            agg = scatter_add(msg, src, dim=0, dim_size=H.size(0))
            out.append(agg)
